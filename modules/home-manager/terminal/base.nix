    ...
    let
      ...
      fishShellFunctions = {
        ...
        nb = {
          body = ''
            ...
            set -l args $argv[2..-1]
            set -l cmd "${lib.getExe pkgs.nom} build --print-build-logs $args '.?submodules=1#$derivation'"
            echo "> $cmd"
            eval "$cmd"
          '';
        ...
        nd = {
          body = ''
            set -l cmd "${lib.getExe pkgs.nom} develop --print-build-logs '.?submodules=1#$argv'"
            echo "> $cmd"
            eval "$cmd"
          '';
      ...
      shellExtra = ''
        ...
        nb() {
          ...
          args=''${arg_array[@]:1}
          local cmd="${lib.getExe pkgs.nom} build --print-build-logs ''${args} '.?submodules=1#''${derivation}'"
          echo "> $cmd"
          eval "$cmd"
        }

        nd() {
          local cmd="${lib.getExe pkgs.nom} develop --print-build-logs '.?submodules=1#$@'"
          echo "> $cmd"
          eval "$cmd"
        }
